      ...
      <div className="bg-ivory border border-cream rounded-2xl p-5 space-y-4 shadow-[rgba(0,0,0,0.05)_0px_4px_24px]">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setMinutes(m => Math.max(0, m - 5))}
              ...
              className="w-7 h-7 rounded-lg border border-cream bg-sand text-charcoal hover:bg-sand/80
        ...
        {includeHistory && (
          <div className="flex items-center justify-between animate-fade-in">
            <p className="text-xs text-bark">How many wrong questions to add</p>
            <div className="flex items-center gap-2">
              <button onClick={() => setHistoryCount(n => Math.max(1, n - 1))}
                aria-label="Decrease history question count"
                className="w-6 h-6 rounded border border-cream bg-sand text-xs text-charcoal
                  hover:bg-sand/80 active:scale-95 transition-all">−</button>
